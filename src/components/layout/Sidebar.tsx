      ...
      <nav className="navigation-list">
        ...
        <p className="nav-label nav-label-spaced">Administration</p>
          {visibleItems.slice(7).map((item) => {
            const Icon = item.icon
            return item.status === 'ready' ? <NavLink key={item.label} to={item.href} onClick={onNavigate} className={({ isActive }) => cn('navigation-link', isActive && 'navigation-link-active')}><Icon size={17} aria-hidden="true" /><span>{item.label}</span><ChevronRight className="nav-chevron" size={14} aria-hidden="true" /></NavLink> : <div key={item.label} className="navigation-link navigation-link-disabled" aria-disabled="true"><Icon size={17} aria-hidden="true" /><span>{item.label}</span><span className="planned-label">Soon</span></div>
        })}
      </nav>
